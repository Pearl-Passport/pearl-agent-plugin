...
test("resource response is versioned, correctly typed, and deny-by-default", async () => {
  ...
  assert.equal(PEARL_MCP_APP_VERSION, "1.6.1");
  assert.equal(content.uri, "ui://pearl/concierge/v15/index.html");
...
test("reviewed action tools are visible only to the model and cards cannot execute them", async () => {
  ...
  const html = await buildHtml();
  assert.doesNotMatch(html, /data\.action_handle(?!_expires_at)|idempotency_key/);
  assert.ok(html.includes("tool?.annotations?.readOnlyHint === true"));
