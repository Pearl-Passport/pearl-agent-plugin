...
The Codex, Claude, and Cursor manifests, marketplace entries, package metadata, MCP Registry `server.json`, and validators use one semantic version. Package `0.8.9` is the current sanitized read-only release candidate.

| Surface | Version | Release tag | Publication boundary |
| --- | --- | --- | --- |
| Codex, Claude, Cursor, and shared skill | `0.8.9` | `v0.8.9` | Separate host installation or review |
| MCP Registry metadata | `0.8.9` | `v0.8.9` | Protected Registry OIDC job after the reviewed host-package release |
...
4. Confirm `server.json` uses the exact `2025-12-11` schema, case-sensitive GitHub namespace `io.github.Pearl-Passport/pearl-agent-plugin`, stable public repository ID `1343507179`, version `0.8.9`, and exactly one `streamable-http` remote with no headers, variables, credentials, or package declaration.
