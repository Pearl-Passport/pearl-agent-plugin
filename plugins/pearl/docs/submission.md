...
Changes to submitted listing information or imported skills require a new version, review and publication in the OpenAI portal. Published MCP tool metadata is separately subject to OpenAI's automated continuous review; held updates keep the previous definition live. Neither process grants new Pearl permissions. See [OpenAI's published metadata guidance](https://developers.openai.com/plugins/deploy/submission#how-published-mcp-metadata-versions-work).

For the current OpenAI contract, scan the same reviewed endpoint and verify the exact 18-tool inventory: 13 common reads, read-only `reservations_availability`, and the four confirmed visit tools. The two prepare calls and import commit are non-destructive writes; visit update commit is destructive because it changes an existing record. Test approval prompts, prepare/confirm separation, idempotent retries, scope denial, and the negative provider-booking canary in the draft before submission. A membership-copy update does not add tools, scopes or client eligibility. Do not copy the separate Codex/Claude/Cursor save/trip capabilities into the OpenAI listing without their own eligibility review and host tests.
